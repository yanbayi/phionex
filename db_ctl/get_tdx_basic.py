...
def fetch_tdx_index_data(pro,ts_codes: List[str] = None,trade_date: str = None,idx_types: List[str] = None) -> pd.DataFrame:
    # 处理默认交易日期
    if not trade_date:
        trade_date = utils.get_end_date(pro, datetime.now())
        logger.info(f"未指定交易日期，使用默认日期: {trade_date}")
    all_data = []
    if idx_types:
        for idx_type in tqdm(idx_types, desc="拉取不同类型板块", unit="类型"):
            try:
                ...
                if not df.empty:
                    all_data.append(df)
                    # logger.info(f"成功获取{idx_type}类型板块{len(df)}条数据")
            except Exception as e:
                logger.error(f"获取{idx_type}类型板块失败: {str(e)}")
    else:
        logger.warning("未指定板块代码和板块类型，无法拉取数据")
        return pd.DataFrame()
...
def fetch_tdx_member_data(pro,ts_codes: List[str] = None,trade_date: str = None) -> pd.DataFrame:
    if not ts_codes:
        logger.warning("未提供板块代码列表，无法拉取成分股数据")
        return pd.DataFrame()
    ...
    for i in tqdm(range(0, len(ts_codes)),desc="拉取成分股数据",total=len(ts_codes),unit="批"):
        try:
            ...
            logger.error(f"获取板块{ts_codes[i]}的成分股失败: {str(e)}")

    # 合并数据并补充字段
    if all_data:
        result_df = pd.concat(all_data, ignore_index=True)
        result_df["up_time"] = datetime.now()  # 数据获取时间
        logger.info(f"共获取到{len(result_df)}条成分股信息")
        return result_df
    return pd.DataFrame()
...
def insert_tdx_index_to_mongodb(data: pd.DataFrame, tdx_index_coll) -> int:
    if data.empty:
        logger.warning("没有数据需要插入MongoDB")
        return 0
    # 转换为字典列表并处理空值
    ...
    for doc in data_dict:
        try:
            ...
            # 捕获并记录验证失败的错误
            logger.error(f"插入记录失败 (ts_code: {doc.get('ts_code')}, trade_date: {doc.get('trade_date')}): {str(e)}")

    logger.info(f"插入完成：新增{inserted_count}条，更新{len(data_dict) - inserted_count}条")
    return inserted_count


def insert_tdx_member_to_mongodb(data: pd.DataFrame, tdx_member_coll) -> int:
    if data.empty:
        logger.warning("没有成分股数据需要插入MongoDB")
        return 0
    # 转换为字典列表并处理空值
    data_dict = data.to_dict("records")
    for doc in data_dict:
    ...
    logger.info(f"成分股插入完成：新增{inserted_count}条，更新{len(data_dict) - inserted_count}条")
    return inserted_count

def main_get_tdx_basic():
    logger.info("=" * 60)
    logger.info("启动tdx模块信息、成分每日全量更新脚本")
    logger.info("=" * 60)
    ...
    try:
        ...
        if not index_df.empty:
            ...
            logger.info(f"共获取到{len(board_codes)}个概念板块，准备拉取成分股")
        else:
            logger.error("未获取到任何板块信息，终止流程")

        # 4. 提取板块代码和日期，用于拉取成分股
        board_codes = index_df["ts_code"].unique().tolist()
        trade_date = index_df["trade_date"].iloc[0]
        logger.info(f"共获取到{len(board_codes)}个板块，准备拉取{trade_date}的成分股数据")
        ...
        if not member_df.empty:
            ...
            logger.warning("未获取到有效成分股数据")


        logger.info("=" * 60)
        logger.info("通达信板块数据处理完成 ！")
        logger.info("=" * 60)
    except Exception as e:
        logger.error("=" * 60)
