...
if __name__ == "__main__":
    ...
    try:
        ...
        time2 = time.time()
        # 获取日线 90天
        # get_daily_share.main_get_daily_share(360, False) # 全部删除，全部更新，适合第一次运行或者修改了获取天数
        get_daily_share.main_get_daily_share(360, True) # 只更新最新的
        time3 = time.time()
        # 获取通达信概念板块信息
        get_tdx_basic.main_get_tdx_basic()
        time4 = time.time()
        # 获取通达信概念板块日线 60天
        # get_tdx_daliy.main_get_tdx_daily(360, False)  # 全部删除，全部更新，适合第一次运行或者修改了获取天数
        get_tdx_daliy.main_get_tdx_daily(360, True) # 只更新最新的
        time5 = time.time()
